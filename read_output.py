...
data_df = data_df.drop(data_df[data_df['diff'] > data_df['diff'].std() * 5 ].index)
data_df = data_df.drop(data_df[data_df['diff'] < np.negative(data_df['diff'].std() * 5) ].index)
...
plt.yticks(x_tick)
plt.grid(True)
plt.text(40, 28, 'Bias = '+str(round(data_df['diff'].mean(), 3)))
plt.text(40, 26, 'STD = '+str(round(data_df['diff'].std(), 3)))
plt.text(40,24,'Sample# = ' + str(data_df.shape[0]))
plt.text(40,30,'RMSE = ' + str(round(np.sqrt(mean_squared_error(data_df['hy_value'], data_df['other_value'])),3)))
plt.text(40,32,'R^2 = ' + str(round(r2_score(data_df['hy_value'], data_df['other_value']),3)))
...
plt.savefig(r'output/'+ str( years) + str(months)  +satelite1+' & '+ satelite2 +' '+ var +' '+ 'scatter_plot.jpg')
plt.show()
...
plt.savefig(r'output/'+ str(years) + str(months)  +satelite1+' & '+satelite2 + var +' '+ 'location_plot.jpg')
